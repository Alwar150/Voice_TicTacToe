...
void SpeechManager::init() {
    if ((err_ = Pa_Initialize()) != paNoError)
        ...
        if (defaultDevice == paNoDevice) {
            ...
            E_FATAL("No default input device found. Please check microphone configuration.");
        }

        inputParameters.device = defaultDevice;
        inputParameters.channelCount = 1;
        inputParameters.sampleFormat = paInt16;
        // Use the low latency suggested by the driver (best practice for real-time)
        inputParameters.suggestedLatency = Pa_GetDeviceInfo(defaultDevice)->defaultLowInputLatency;
        inputParameters.hostApiSpecificStreamInfo = NULL;

        if ((err_ = Pa_OpenStream(&stream_, &inputParameters, NULL,
                                  ps_config_int(config_, "samprate"),
                                  frame_size_, paClipOff, NULL, NULL)) != paNoError)
        {
            // Re-introduce the detailed failure diagnostic if the explicit open fails
            fprintf(stderr, "PortAudio Stream Error (Code %d): %s\n",
                    err_, Pa_GetErrorText(err_));
            const PaHostErrorInfo* hostErr = Pa_GetLastHostErrorInfo();
            if (hostErr) {
                E_FATAL("Failed to open PortAudio stream. Host Error: %s\n", hostErr->errorText);
            } else {
